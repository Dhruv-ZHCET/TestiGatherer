...
function Space() {
	...
	const handleliked = async (testimonialid, isLiked) => {
		try {
			...
			const response = await axios.post(
				'http://localhost:3001/api/v1/liked',
				{
					testimonialid,
					isLiked: !isLiked,
				},
			...
			setTestimonials(
				testimonials.map((t) =>
					t.id === testimonialid ? { ...t, liked: isLiked } : t
				)
			);
		}
	};
	...
	const filteredTestimonials = testimonials.filter(testimonial => {
		switch (selectedCategory) {
			...
			case 'text':
				return true; // Assuming all testimonials are text for now
			default:
		...
		<div className='w-full flex flex-col justify-center text-white font-bold bg-[#151719]'>
			...
			<div className='px-4 pb-6 flex'>
				...
				<div className='w-1/5 flex flex-col'>
					{/* Inbox Section - Updated with click handlers */}
					<ul className='flex flex-col gap-1 mt-12'>
						...
						<li>
							<button
								...
								onClick={() => handleCategoryClick('liked')}
							>
					...
					{/* Rest of the sidebar sections - Unchanged */}
				...
				<div className='w-4/5 flex flex-col gap-10 pt-16 items-center px-32'>
					...
					{/* Testimonials List - Updated to use filtered testimonials */}
